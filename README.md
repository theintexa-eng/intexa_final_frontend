...
- Optional: set `VITE_API_BASE_URL` in `.env.local`.
	- Example local backend: `VITE_API_BASE_URL=http://localhost:3000`
- If `VITE_API_BASE_URL` is not set, frontend uses relative paths like `/api/inquiry/start`.
- Vite dev server proxy is configured in `vite.config.js` to forward `/api/*` to `http://localhost:3000`.
